################################################
###                                          ###
###  Robustness check: regular price index   ###
###                                          ###
################################################
...
heckman_main <- selection(selection = !is.na(Total_volume_sales) ~  PromoIndex + PanicIndex
                          ...
                          + RegularPriceIndex + Age + HouseholdSize + Income + Day_of_week + Week + Year,
...
keep <- c("PromoIndex","PanicIndex", "HouseholdInventory","Average_IPT","Average_Q", "RegularPriceIndex", "Age", "HouseholdSize", "Income")
# save clean output
two_stages_output_clean(heckman_main, "Heckman-regular-price-index-robustness-check", keep)
...
fe_mod <- plm(unconditional_Total_volume_sales ~  PromoIndex + PanicIndex +
                ...
                # controls
                RegularPriceIndex + Day_of_week + Week + Year,
...
# save clean output
fixed_effects_output_clean(fe_mod_clus, "Fixed-Effects-regular-price-index-robustness-check", keep_mod)
