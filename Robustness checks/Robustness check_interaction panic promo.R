################################################################
###                                                          ###
###  Robustness check: interacting Promo and Panic Indices   ###
###                                                          ###
################################################################
...
heckman_main <- selection(selection = !is.na(Total_volume_sales) ~  PromoIndex + PanicIndex + PromoIndex:PanicIndex
                          + HouseholdInventory + Average_IPT 
                          + PriceIndex + Age + HouseholdSize + Income + Day_of_week + Week + Year,
                          outcome =          Total_volume_sales  ~  PromoIndex + PanicIndex + PromoIndex:PanicIndex
                          + HouseholdInventory + Average_Q   
                          + PriceIndex + Age + HouseholdSize + Income + Day_of_week + Week + Year,
                          data = FullData)
# clean version: excluding time fixed effects from output
keep <- c("PromoIndex","PanicIndex","PromoIndex:PanicIndex", "HouseholdInventory","Average_IPT","Average_Q", "PriceIndex", "Age", "HouseholdSize", "Income")
# save clean output
two_stages_output_clean(heckman_main, "Heckman-panic-promo-interaction-robustness-check", keep)
...
fe_mod <- plm(unconditional_Total_volume_sales ~  PromoIndex + PanicIndex + PromoIndex:PanicIndex +
...
# save clean output
fixed_effects_output_clean(fe_mod_clus, "Fixed-Effects-panic-promo-interaction-robustness-check", keep_mod)
