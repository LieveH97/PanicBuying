...
table(TP_purch$Purchase_method)
table(TP_purch$Purchase_method[TP_purch$Year == "2018"])
table(TP_purch$Purchase_method[TP_purch$Year == "2019"])
table(TP_purch$Purchase_method[TP_purch$Year == "2020"])



online_flags <-  c("online-bezorgen", "online-ophalen")
TP_purch_noonline <- TP_purch[!TP_purch$Purchase_method %in% online_flags,]
rm(TP_purch)
...
filter_minpurch <- filter_minpurchase_year(TP_purch_noonline,yearlist)
filter_minpurch <- filter_minpurch[filter_minpurch$filterOK==1,]
# apply filter to TP_purch_noonline dataset
TP_purch_noonline <- TP_purch_noonline[TP_purch_noonline$Panelist %in% filter_minpurch$ID,]
...
#TP_purch_noonline_agg_d <- ddply(TP_purch_noonline,.(Panelist,Date_of_purchase, Quarter, Year),
...
TP_purch_noonline_agg_d <- TP_purch_noonline_agg_d[order(TP_purch_noonline_agg_d$Panelist,TP_purch_noonline_agg_d$Date),]
TP_purch_noonline_agg_d$IPT<-do.call(c,by(TP_purch_noonline_agg_d$Date,TP_purch_noonline_agg_d$Panelist,function(x) c(NA,diff(x))))
...
TP_purch_noonline_agg_d$flag <- ifelse(TP_purch_noonline_agg_d$Date_of_purchase < as.Date("2020-01-01"), "before", "during/after")
# add December 31, 2019 as a zero-purchase in TP_purch_noonline_agg_d
# TP_purch_noonline_agg_d_cut <- rbind(TP_purch_noonline_agg_d,  cbind(Panelist=as.character(unique(TP_purch_noonline_agg_d$Panelist)), 
#                                                Date_of_purchase="2019-12-31", Quarter="201904", Year="2019", 
#                                                tot_sheet=0,IPT=NA,flag="before"))
TP_purch_noonline_agg_d_cut <- rbind(TP_purch_noonline_agg_d,  cbind(Panelist=as.character(unique(TP_purch_noonline_agg_d$Panelist)),
                                                   Date_of_purchase="2019-12-31", Quarter="201904", Year="2019",
                                                   tot_rolls=0,IPT=NA,flag="before"))
TP_purch_noonline_agg_d_cut <- TP_purch_noonline_agg_d_cut[order(TP_purch_noonline_agg_d_cut$Panelist,TP_purch_noonline_agg_d_cut$Date),]
TP_purch_noonline_agg_d_cut$IPT_cut <- do.call(c,by(TP_purch_noonline_agg_d_cut$Date,TP_purch_noonline_agg_d_cut$Panelist,function(x) c(NA,diff(x))))
# add flag
TP_purch_noonline_agg_d_cut$flag <- ifelse(TP_purch_noonline_agg_d_cut$Date_of_purchase < as.Date("2020-01-01"), "before2020", "2020")
max_IPT <-aggregate(IPT_cut ~ Panelist + flag, TP_purch_noonline_agg_d_cut, max)
...
panelists_drop_ID_cat <- as.vector(unique(TP_purch_noonline$Panelist[!TP_purch_noonline$Panelist %in% filter_IPT_2020_st$Panelist]))
...
HH <- HH[HH$Panelist %in% unique(TP_purch_noonline$Panelist),]
...
TP_purch_noonline <- TP_purch_noonline[TP_purch_noonline$Panelist %in% panelists_keep_ID_cat,]
# only after analysis of kept vs. dropped! otherwise HH characteristics of dropped will be empty
# because first "only keep HH info of HH in TP_purch_noonline" -> TP_purch_noonline shouldn't be filtered yet!

TP_purch_noonline_agg_d <- TP_purch_noonline_agg_d[TP_purch_noonline_agg_d$Panelist %in% panelists_keep_ID_cat,]
TP_purch_noonline_agg_d_cut <- TP_purch_noonline_agg_d_cut[TP_purch_noonline_agg_d_cut$Panelist %in% panelists_keep_ID_cat,]

save(panelists_keep_ID_cat, file=paste(project_path,"/R_files/Preparation/toilet paper/",country,"_IPT_HH_filter_noonline.RData",sep=""))
...
# TP_purch_noonline_agg_y <- ddply(TP_purch_noonline, .(Panelist,Year), summarise, tot_sheet = sum(tot_sheet),
...
TP_purch_noonline_agg_y <- ddply(TP_purch_noonline, .(Panelist,Year), summarise, tot_rolls = sum(Total_volume_sales))
...
### AVERAGE PRICES ###
TP_purch_noonline$AvgPrice_pack <- TP_purch_noonline$Total_value_sales / TP_purch_noonline$Total_unit_sales * 0.01
TP_purch_noonline$AvgPrice_roll <- TP_purch_noonline$Total_value_sales / TP_purch_noonline$Total_volume_sales * 0.01
#TP_purch_noonline$AvgPrice_200sheet <- TP_purch_noonline$AvgPrice_roll / TP_purch_noonline$sheet * 200
# average price of pack, per ply type (packs can contain different number of rolls, of different size!)
ddply(TP_purch_noonline, "ply", summarise, avg_price_pack=mean(AvgPrice_pack))
# average price of roll, per ply type (rolls can be of different size!)
ddply(TP_purch_noonline, "ply", summarise, avg_price_roll = mean(AvgPrice_roll))
# average price of 200 sheet roll, per ply type
#ddply(TP_purch_noonline, "ply", summarise, avg_price_200sheet = mean(AvgPrice_200sheet))

### PLY ###
# SKUs per ply type
ggplot(toiletpaper, aes(x=factor(ply))) + geom_bar(position="dodge", show.legend=F)
# calculating market shares of "ply type" per year
ply_MS <- ddply(TP_purch_noonline, .(Year, ply), summarise, Total_unit_sales = sum(Total_unit_sales),
                Total_volume_sales = sum(Total_volume_sales), Total_value_sales = sum(Total_value_sales))
year_sum <- ddply(TP_purch_noonline, .(Year), summarise, Total_unit_sales = sum(Total_unit_sales),
                 Total_volume_sales = sum(Total_volume_sales), Total_value_sales = sum(Total_value_sales))
year_sum <- (do.call("rbind", replicate(length(unique(TP_purch_noonline$ply)), year_sum, simplify = FALSE)))
...
# sheet_MS <- ddply(TP_purch_noonline, .(Year, sheet), summarise, Total_unit_sales = sum(Total_unit_sales),
#                Total_volume_sales = sum(Total_volume_sales), Total_value_sales = sum(Total_value_sales))
# year_sum <- ddply(TP_purch_noonline, .(Year), summarise, Total_unit_sales = sum(Total_unit_sales),
#                  Total_volume_sales = sum(Total_volume_sales), Total_value_sales = sum(Total_value_sales))
...
PL_MS <- ddply(TP_purch_noonline, .(Year, PL), summarise, Total_unit_sales = sum(Total_unit_sales),
               Total_volume_sales = sum(Total_volume_sales), Total_value_sales = sum(Total_value_sales))
year_sum <- ddply(TP_purch_noonline, .(Year), summarise, Total_unit_sales = sum(Total_unit_sales),
                 Total_volume_sales = sum(Total_volume_sales), Total_value_sales = sum(Total_value_sales))
year_sum <- (do.call("rbind", replicate(length(unique(TP_purch_noonline$PL)), year_sum, simplify = FALSE)))
year_sum <- year_sum[order(year_sum$Year),]
...
# PER DAY
tot_Q <- ddply(TP_purch_noonline, .(Date_of_purchase, Year), summarize, Tot_unit_sales = sum(Total_unit_sales),
...
# PER IFC WEEK
tot_Q <- ddply(TP_purch_noonline[!is.na(TP_purch_noonline$Week_IFC),], .(Week_IFC, Year), summarize, Tot_unit_sales = sum(Total_unit_sales),
              Tot_volume_sales=sum(Total_volume_sales), Tot_value_sales=sum(Total_value_sales))
tot_Q$avgP <- tot_Q$Tot_value_sales / tot_Q$Tot_volume_sales
...
# PER DAY
Q_per_HH <- ddply(TP_purch_noonline, .(Date_of_purchase, Year), summarize, Tot_volume_sales=sum(Total_volume_sales), HH_buying = length(Date_of_purchase))
...
# PER IFC WEEK
Q_per_HH <- ddply(TP_purch_noonline[!is.na(TP_purch_noonline$Week_IFC),], .(Week_IFC, Year), summarize, Tot_volume_sales=sum(Total_volume_sales), HH_buying = length(Week_IFC))
Q_per_HH$Q_HH <- Q_per_HH$Tot_volume_sales / Q_per_HH$HH_buying
...
PrevPurch <- aggregate(IPT ~ Date_of_purchase, TP_purch_noonline_agg_d, mean)
...
# PER YEAR #
avg_daily_cons <- data.frame(HH_ID = unique(TP_purch_noonline_agg_d$Panelist))
days_2018 <- 365
days_2019 <- 365
days_2020 <- 366

# for (i in seq_along(avg_daily_cons$HH_ID)) {
#  avg_daily_cons$Y2018[i] <- TP_purch_noonline_agg_y$tot_sheet[TP_purch_noonline_agg_y$Panelist==avg_daily_cons$HH_ID[i] & TP_purch_noonline_agg_y$Year==2018] / days_2018
#  avg_daily_cons$Y2019[i] <- TP_purch_noonline_agg_y$tot_sheet[TP_purch_noonline_agg_y$Panelist==avg_daily_cons$HH_ID[i] & TP_purch_noonline_agg_y$Year==2019] / days_2019
#  avg_daily_cons$Y2020[i] <- TP_purch_noonline_agg_y$tot_sheet[TP_purch_noonline_agg_y$Panelist==avg_daily_cons$HH_ID[i] & TP_purch_noonline_agg_y$Year==2020] / days_2020
# }
for (i in seq_along(avg_daily_cons$HH_ID)) {
  avg_daily_cons$Y2018[i] <- TP_purch_noonline_agg_y$tot_rolls[TP_purch_noonline_agg_y$Panelist==avg_daily_cons$HH_ID[i] & TP_purch_noonline_agg_y$Year==2018] / days_2018
  avg_daily_cons$Y2019[i] <- TP_purch_noonline_agg_y$tot_rolls[TP_purch_noonline_agg_y$Panelist==avg_daily_cons$HH_ID[i] & TP_purch_noonline_agg_y$Year==2019] / days_2019
  avg_daily_cons$Y2020[i] <- TP_purch_noonline_agg_y$tot_rolls[TP_purch_noonline_agg_y$Panelist==avg_daily_cons$HH_ID[i] & TP_purch_noonline_agg_y$Year==2020] / days_2020
}
...
# AvgDailyCons <- TP_purch_noonline_agg_d[,c("Panelist","Date_of_purchase","tot_sheet")]
AvgDailyCons <- TP_purch_noonline_agg_d[,c("Panelist","Date_of_purchase","tot_rolls")]
names(AvgDailyCons) <- c("Panelist","Date","Q")
# create dataframe of all dates-HH combos
HH_in_panel <- unique(TP_purch_noonline$Panelist)
...
avg_IPT_year <- aggregate(IPT~Panelist+Year, TP_purch_noonline_agg_d, mean)
avg_IPT_year_2019 <- subset(avg_IPT_year, Year=="2019", select= c("Panelist","IPT"))
avg_IPT_19 <- aggregate(IPT~Panelist, subset(TP_purch_noonline_agg_d, Year=="2019"), mean)
avg_IPT_1819 <- aggregate(IPT~Panelist, subset(TP_purch_noonline_agg_d, Year=="2018"|Year=="2019"), mean)
# max IPT calculated across 2018-2019, with hard cut at Dec 31,2019
max_IPT_before2020 <- aggregate(IPT_cut~Panelist, subset(TP_purch_noonline_agg_d_cut, Year=="2018"|Year=="2019"), max)
...
if (country == "NL") {TP_purch_noonline <- TP_purch_noonline[,c("Date_of_purchase","Barcode", "Panelist",
                                           "Banner_name", "Total_unit_sales","Total_value_sales",
                                           "Total_volume_sales", "Quarter", "Year", "Promo","PL",
                                           "ply","sheet")] }
save(TP_purch_noonline, file=paste(project_path,"/R_files/Preparation/toilet paper/",country,"_purch_toiletpaper_noonline.RData",sep=""))
save(Inv, file=paste(project_path,"/R_files/Preparation/toilet paper/",country,"_INV_toiletpaper_noonline.RData",sep=""))
